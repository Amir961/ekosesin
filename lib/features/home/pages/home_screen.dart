...
class _HomeScreenState extends State<HomeScreen> {
  ...
  getData(bool isFirst, {String? searchValue}){
    ...
    if(BlocProvider.of<citBloc.CityBloc>(context).state.selectedCity!=null && BlocProvider.of<citBloc.CityBloc>(context).state.selectedCity?.id!=-1) {
      variables.addAll({"city_id": BlocProvider.of<citBloc.CityBloc>(context).state.selectedCity?.id});
    }
  ...
  getDataCategory(){
    ...
    BlocProvider.of<catBloc.CategoryBloc>(context).add(catBloc.GetDataEvent(variables));
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: SafeArea(child: Column(
        children: [
          HeaderWidget(hint: Strings.of(context).hint_search_header_label, changeSearch: (String value ) {
      ...
      bottomNavigationBar:  ButtomSheetWidget(changeCategory: (){
        getData(true);
      },)
  ...
  Widget get _list => BlocListener<citBloc.CityBloc,citBloc.CityState>(
      ...
      listenWhen: (previous, current) =>(previous.selectedCity != current.selectedCity) && previous.selectedCity!=null,
       ...
       child: BlocConsumer<HomeBloc,HomeState>(
        ...
        listener: (BuildContext context, HomeState state) {
          if(state.statusButton == StatusButton.noInternet)
            {
              sl<DialogManager>().showNoNetDialog(context: context, onTryAgainClick: (){
                getData(false);
              });
            }
          if(state.statusButton == StatusButton.failed)
          {
            sl<DialogManager>().showErrorDialog(context: context, onTryAgainClick: (){
              getData(false);
            }, message: state.message);
          }
        },

          buildWhen: (previous, current) =>previous.statusButton != current.statusButton ,
        ...
        children: [
          ...
          Expanded(child: Padding(
            ...
            child: Column(
              ...
              children: [
                MyText(text: product.translateTitle()),
                MyText(text: '${product.currency?.sign} ${product.price.toString()}'),
                Row(
                  children: [
                ...
                Row(
                  ...
                  children: [
                    Expanded(child: MyText(text: product.createdAt?.split('T')[0]??'',fontSize: 12,)),
                    Container(
                     ...
                     decoration: BoxDecoration(
                      ...
                      child: Center(child: MyText(text: product.category?.translateTitle()??'',fontSize: 11,)),
                    )
    ...
    Row(
      ...
      children: [
          ...
          sl<DialogManager>().showErrorDialog(context: context, onTryAgainClick: (){
            getDataCategory();
              ...
              builder: (context,state) {
                ...
                return
          state.statusButton == StatusButton.loading?
          LoadingWidget(): MyText(text: state.queenList.isEmpty? Strings.of(context).description_sorted:state.queenList.last.translateMetaTitle());
        ...
        BlocConsumer<SortedBloc,SortedState>(
            ...
            listenWhen: (previous, current) =>(previous.selectedSortedClass != current.selectedSortedClass),



            buildWhen: (previous, current) =>previous.selectedSortedClass != current.selectedSortedClass ,
          builder: (context,state) {
            ...
            return DropdownButtonHideUnderline(
              ...
              child: DropdownButton2<SortedClass>(
                ...
                items: state.list
                    .map((SortedClass item) => DropdownMenuItem<SortedClass>(
                  value: item,
                  child: Row(
...
class SortedClass {
 ...
 SortedClass({required this.index, required this.sortBy, required this.sortDir});
